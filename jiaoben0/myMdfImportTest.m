function varargout = myMdfImport(varargin)
...
if isequal(fileName,0)|isequal(pathName,0) %#ok
    ...
    switch fileName(end-3:end) % Look at file type
        ...
        case '.txt'
            ...
            % Load text file
%wwg            requestedChannelList=readtextfile([pathName fileName]);
...
for dataBlock=selectedDatablocks % For each (either one or all)
    ...
    % Find time channel
    timeChannel=findtimechannel(MDFInfo.DGBlock(dataBlock).CGBlock(1).TXBlock);
 ...
 %wwg                   varName= [signalNames{k} '_' varEnding additionalText];
                    varName= [signalNames{k} '_' varEnding];
                case 'ratestring'
...
% % %             % wwg start: assign things in caller
% % %             ws='caller';
% % %             % wwg end
...
%                       'Name',MDFInfo.DGBlock(dataBlock).CGBlock.TXBlock(k).longSignalName);
%                    ts1.dataInfo.Units=MDFInfo.DGBlock(dataBlock).CGBlock.TXBlock(k).CCBlock.physicalUnit; % Add units
%                    ts1.dataInfo.UserData=MDFInfo.DGBlock(dataBlock).CGBlock.TXBlock(k).signalDescription; % description
...
for dataGroup=1:double(MDFstructure.HDBlock.numberOfDataGroups) % Work for older versions
    ...
    offset=CGBlockTemp.pointerToFirstTXBlock;

    % For each Channel
    for channel=1:double(CGBlockTemp.numberOfChannels)

        %%% Read Channel block (TXBlock) information - offset set already

        % Read data Channel block info into structure

        TXBlockTemp=mdfblockread(formats.CNFormat,fid,offset,1);

        % Convert blockIdentifier, signalName, and signalDescription
        % string data from uint8 to char
        TXBlockTemp.signalName=truncintstochars(TXBlockTemp.signalName);
        TXBlockTemp.signalDescription=truncintstochars(TXBlockTemp.signalDescription);

        %%% Read Channel text block (TXBlock)

        offset=TXBlockTemp.pointerToTXBlock1;
        if double(offset)==0
            TXBlockTemp=struct('blockIdentifier','NULL','blocksize', 0);
            TXBlockTemp.longSignalName='';
        else
            % Read data Text block info into structure
            TXBlockTemp=mdfblockread(formats.TXFormat,fid,offset,1);
            ...
            if TXBlockTemp.blockSize>0 % If non-zero (check again)
                ...
                TXBlockTemp.blockIdentifier=truncintstochars(TXBlockTemp.blockIdentifier);
                TXBlockTemp.comment=truncintstochars(TXBlockTemp2.comment); % accessing TXBlockTemp2
                TXBlockTemp.longSignalName=TXBlockTemp.comment;
        ...
        % Copy temporary Text block info into main MDFstructure
        TXBlockTemp.TXBlock=TXBlockTemp;
        % NOTE: This could be removed later, only required for long name which
        % gets stored in structure seperately

        if TXBlockTemp.blockIdentifier==7 % If text only
            offset=TXBlockTemp.pointerToConversionFormula;
        ...
        % block info
        TXBlockTemp.CCBlock=CCBlockTemp;

        % Get pointer to next Channel block ready for next loop
        offset=TXBlockTemp.pointerToNextTXBlock;

        % Copy temporary Channel block info into temporary Channel
        % Group info
        CGBlockTemp.TXBlock(channel,1)=TXBlockTemp;
    end
    
    % Sort channel list before copying in because sometimes the first
    % channel is not listed first in the block
    pos=zeros(length(CGBlockTemp.TXBlock),1);
    for ch = 1: length(CGBlockTemp.TXBlock)
        pos(ch)=CGBlockTemp.TXBlock(ch).blockSize; % Get start bits
    end
    
    [dummy,idx]=sort(pos); % Sort positions to getindices
    clear TXBlockTemp2
    for ch = 1: length(CGBlockTemp.TXBlock)
        TXBlockTemp2(ch,1)= CGBlockTemp.TXBlock(idx(ch)); % Sort blocks
    end
    
    % Copy sorted blocks back
    CGBlockTemp.TXBlock=TXBlockTemp2;
...
formats.CGFormat={...
    UINT8  [1 8]  'ignore';...
    LINK   [1 1]  'pointerToFirstTXBlock';  ...
...
channelGroup=1;
fieldNames=fieldnames(MDFStructure.DGBlock(1).CGBlock(channelGroup).TXBlock);
startChannel=1;
...
for dataBlock = 1: numberOfDataGroups
    ...
    summary(dataBlock,1).rateVariableSampled=MDFStructure.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(1).rateVariableSampled;
    channelCells=[fieldNames struct2cell(MDFStructure.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock)];
    ...
    % Make channel List
    channelCells2=struct2cell(MDFStructure.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock);
    ...
    channelList(startChannel:endChannel,5)={MDFStructure.DGBlock(dataBlock).CGBlock.TXBlock(1).rateVariableSampled};
...
for channel=1:numberOfChannels
    numberOfBits= MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).numberOfBits;
    blockIdentifier= MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).blockIdentifier;
    datatype=datatypeformat(blockIdentifier,numberOfBits); %Get signal data type (e.g. 'int8')
    if blockIdentifier==7 % If string
...
timeChannel=findtimechannel(MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock);
...
for selectedChannel=1:length(selectedChannels)
    ...
    % Get signal names
    longSignalName=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).longSignalName;
    if ~isempty(longSignalName) % if long signal name is not empty use it
        ...
        signalNames{selectedChannel,1}=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).signalName;
    end

    if MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).blockIdentifier==7
        % Strings: Signal Data Type 7
        data{selectedChannel}=truncintstochars(Blockcell{selectedChannel}); % String
    elseif MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).blockIdentifier==8
        % Byte arrays: Signal Data Type 8
        error('MDFReader:signalType8','Signal data type 8 (Byte array) not currently supported');
        
%     elseif MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).valueRangeKnown % If physical value is correct...
...
%         data{selectedChannel}=double(Blockcell{selectedChannel});
        ...
        conversionFormulaIdentifier=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.conversionFormulaIdentifier;
        ...
        switch conversionFormulaIdentifier
            case 0 % Parameteric, Linear: Physical =Integer*P2 + P1
                ...
                P1=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.P1;
                P2=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.P2;
            ...
            case 1 % Tabular with interpolation
                ...
                int_table=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.int;
                phys_table=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.phys;
            ...
            case 6 % Polynomial
                ...
                P1=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.P1;
                P2=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.P2;
                P3=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.P3;
                P4=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.P4;
                P5=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.P5;
                P6=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.P6;
            ...
            case 10 % ASAM-MCD2 Text formula
                textFormula=MDFInfo.DGBlock(dataBlock).CGBlock(channelGroup).TXBlock(channel).CCBlock.textFormula;
...
function dataType= datatypeformat(blockIdentifier,numberOfBits)
% DATATYPEFORMAT Data type format precision to give to fread
%   DATATYPEFORMAT(blockIdentifier,NUMBEROFBITS) is the precision string to
%   give to fread for reading the data type specified by blockIdentifier and
%   NUMBEROFBITS
...
function timeChannel=findtimechannel(TXBlock)
% Finds the locations of time channels in the channel block
% Take channel blcok array of structures

% % Sort channel list
% position=zeros(length(TXBlock),1);
% for channel = 1: length(TXBlock)
%     position(channel)=TXBlock.numberOfFirstBits;
% end
...
% For each channel
for channel = 1: length(TXBlock)
    if TXBlock(channel).channelType==1; % Check to see if is time
    ...
    for dataBlock=1:numDataBlocks
        ...
        if strcmpi(options.timeVectorChoice,'actual') % If using actual time vectors
            if length(selectedChannelIndices)>=1
                ...
                timechannel=findtimechannel(options.MDFInfo.DGBlock(dataBlock).CGBlock(1).TXBlock);
...
%wwg function requestedChannelList=readtextfile(fileName)
function requestedChannelList=readtextfile(fileName, varMiddleName)
...
while ~feof(fid)
%wwg    requestedChannelList{signalName,1}=fgetl(fid); % Read one line
    requestedChannelList{signalName,1} = [fgetl(fid),varMiddleName]; % Read one line
...
%wwg                requestedChannelList=readtextfile(parameters{3}); % Load file
                requestedChannelList=readtextfile(parameters{3}, parameters{6}); % Load file
