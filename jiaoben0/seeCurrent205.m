function seeCurrent205(inputFileFull, StrExtra4Var, theNumOfFile, filePathOutput)
...
if evalin('base',['exist([''Cyl_3_''', StrExtra4Var, ',''11''], ''var'')']),
    current = eval([...
    ...
    if evalin('base',['exist([''time_''', ',''20''], ''var'')']),  % for Stefan's OPL199
...
elseif evalin('base',['exist([''time_''', ',''9''], ''var'')']) && evalin('base',['exist([''zSwOnResult__ls_0_rs__3''', ',''''], ''var'')']),  % for Andre's OPL014
    ...
    % for GI project time_2 = time_9 (allmost; error<0.15)
    ...
    fprintf('Error: No current for analysis');
    pause
end
...
intervalPlot = floor(5e-5/samplingTime);   % for plotting v-shape piles:~ 10e-5 [s]
...
figure(5); bringFigToFromBackground(5, 'off'); reLocateFigure(gcf, 800, 940); clf;
...
figure(2); bringFigToFromBackground(2, 'off'); reLocateFigure(gcf, 700, 432); clf;
...
for i = 1:numOfCylinder % 1:numOfCylinder
    ...
    subplot(2,4,i, 'FontSize',6); hold('off'); grid('on');
    set(gca, 'Ylim', [0 17]);
    ...
    subplot(2,4,i+4, 'FontSize',6); hold('off'); grid('on');
    ...
    % figure(figHdl4VShapeCollection); clf;
    set(0, 'CurrentFigure',figHdl4VShapeCollection); clf;
    %     bringFigToFromBackground(5, plotInForeground);
    ...
    while j<numOfAllCurrentPoint-0.06*indexWidthSVCurrentEst, %  && eventCounterRaw<=5, % we limit to 15 event here
        ...
        if doV,
            ...
            % find tPeak
            tPeaki      = 1e-6*getSVCAResult(time1(jPeakEnd), 1, time2, tPeak, 4);
            ...
            % figure(figHdl4VShapeCollection);
            ...
            elseif whichCol ~= floor(eventCounterRaw*intervalFactor / numOfVCurvePerCol),
                ...
                plotIntervalMax  = 1.2;
                startPos4Col     = startPos4Col+0.01 + plotIntervalLast;
            ...
            % plot a black bar for time stamps
            if mod(1000*time1(jPeakStart),5000)<8*5000*(time1(jPeakEnd)-time1(jPeakStart)),
                plot([0 0.1]+startPos4Col, eventCounterRaw*intervalFactor-numOfVCurvePerCol*whichCol+[0 0], 'k-', 'LineWidth', 2);
            end
            ...
            % plot delta-I/t threshold
            zSVStuckIThri = getSVCAResult(time1(jPeakEnd), 1, time2, zSVStuckIThr, 4);
            zSVStuckTThri = getSVCAResult(time1(jPeakEnd), 1, time2, zSVStuckTThr, 4)/1000;
            
            %             zSVStuckIThri = getValueAtTime(time1(jHoldEnd), 1, time2, zSVStuckIThr);      % delta-i
            %             zSVStuckTThri = getValueAtTime(time1(jHoldEnd), 1, time2, zSVStuckTThr)/1000; % delta-t
            ...
            if 0,
                ...
                plot([0.04 0.04]+startPos4Col+nValveModeIdi/10*2.4,  eventCounterRaw*intervalFactor-numOfVCurvePerCol*whichCol+[0 1.6], 'Color',[0.8 0.8 0.8], 'LineWidth',2.0);
                % rpm
                [rpmi jStart5] = interpolateFrom(time1(jPeakStart), time5, rpm5, min(1,jStart5));
                plot([0.04 0.04]+startPos4Col+rpmi/3000*2.4,         eventCounterRaw*intervalFactor-numOfVCurvePerCol*whichCol+[0 1.6], 'c-', 'LineWidth',2.0);
                % wOilTemp
                % [wOilTempi jStart5] = interpolateFrom(time1(jPeakStart), time5, wOilTemp5, jHoldEndPrev);
                wOilTempi = wOilTemp5(jStart5);
                plot([0.04 0.04]+startPos4Col+wOilTempi/150*2.4,     eventCounterRaw*intervalFactor-numOfVCurvePerCol*whichCol+[0 1.6], 'y-', 'LineWidth',0.4);
                % iBatteryVolt
                ...
                plot([0.04 0.04]+startPos4Col+iBatteryVolti/20*2.4,  eventCounterRaw*intervalFactor-numOfVCurvePerCol*whichCol+[0 1.6], 'Color',[0.7 0.7 0.7], 'LineWidth',1.0);
                % wCoilTemp
                [wCoilTempi jStart8] = interpolateFrom(time1(jPeakStart), time8, wCoilTemp8, jStart8);
                plot([0.04 0.04]+startPos4Col+wCoilTempi/150*2.4,    eventCounterRaw*intervalFactor-numOfVCurvePerCol*whichCol+[0 1.6], 'g-', 'LineWidth',0.4);
                % eStart
                [eStarti jStart9] = interpolateFrom(time1(jPeakStart), time9, eStart9, jStart9);
                plot([0.04 0.04]+startPos4Col+eStarti/640*2.4,       eventCounterRaw*intervalFactor-numOfVCurvePerCol*whichCol+[0 1.6], 'c-', 'LineWidth',1.0);
                % eFinish
                % [eFinishi jStart9] = interpolateFrom(time1(jPeakStart), time9, eFinish9, jHoldEndPrev);
                eFinishi = eFinish9(jStart9);
                plot([0.04 0.04]+startPos4Col+eFinishi/640*2.4,      eventCounterRaw*intervalFactor-numOfVCurvePerCol*whichCol+[0 1.6], 'y-', 'LineWidth',1.0);
                % peak current
                plot([0.04 0.04]+startPos4Col+current(jPeakStart,i)/18*2.4, eventCounterRaw*intervalFactor-numOfVCurvePerCol*whichCol+[0 1.6], 'g-', 'LineWidth',2.0);
            ...
            title({['Current V-Shapes for cylinder ', num2str(i), ' (data file: ', fileName, ')']; ...
                '(Status/line legend: 0/thin; 1/normal; 5/thick. Time/marker: tSwOn/triangle; tPeak/diamond. Stuck threshold: dot)'}, ...
    ...
    print(figHdl4VShapeCollection, '-dpdf', '-r1200', [filePathOutput, '\current\', fileName, '_V_shape4Cyl', num2str(i)]);
...
% if XP == 3,
%     measAnalDelay = 3; % 3: get SVCA results after 3 TDC's
% elseif XP == 4,
%     measAnalDelay = 3; % 3: get SVCA results after 3 TDC's
% elseif XP ==5,
%     measAnalDelay = 7; % 3: get SVCA results after 7 TDC's
% end
